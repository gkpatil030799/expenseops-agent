...
def test_role_plan_has_exact_attributes_and_no_membership_inheritance():
    ...
    assert (
        "ALTER ROLE expenseops_backup WITH LOGIN NOSUPERUSER BYPASSRLS "
        "NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION"
    ) in plan
...
def test_role_plan_establishes_owner_and_least_privilege_default_acls():
    ...
    assert "REVOKE EXECUTE ON FUNCTIONS FROM expenseops_backup" in plan
...
def test_role_plan_revokes_only_independently_grantable_public_types():
    ...
    assert bootstrap.VERIFY_SQL.count("type_object.typtype <> 'm'") == 2
...
def test_backup_role_is_select_only_and_fail_closed():
    plan = bootstrap.render_plan()

    assert "GRANT USAGE ON SCHEMA public TO expenseops_backup" in plan
    assert "GRANT SELECT ON TABLE %s TO expenseops_backup" in plan
    assert "GRANT SELECT ON SEQUENCE %s TO expenseops_backup" in plan
    assert "GRANT USAGE ON SEQUENCE %s TO expenseops_backup" not in plan
    assert "GRANT EXECUTE ON FUNCTION %s TO expenseops_backup" not in plan
    assert "REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM expenseops_backup" in plan
    assert "REVOKE ALL PRIVILEGES ON TYPES FROM expenseops_backup" in plan
    assert "REVOKE ALL PRIVILEGES ON TABLES FROM expenseops_backup" in plan
    assert "REVOKE ALL PRIVILEGES ON SEQUENCES FROM expenseops_backup" in plan
    assert "expenseops_backup database/schema privileges are unsafe" in plan
    assert "expenseops_backup must not own a database" in plan
    assert "expenseops_backup must not own database objects" in plan
    assert "backup table privileges are unsafe" in plan
    assert "backup sequence privileges are unsafe" in plan
    assert "backup function privilege is unsafe" in plan
    assert "backup has privileges on a public type" in plan
    assert "backup has privileges on an unexpected public relation" in plan
    assert "backup has privileges on an unexpected public sequence" in plan
    assert "backup/default privileges are unsafe for future objects" in plan
    for privilege in ("INSERT", "UPDATE", "DELETE", "TRUNCATE", "REFERENCES", "TRIGGER"):
        assert f"has_table_privilege('expenseops_backup', relation_oid, '{privilege}')" in plan
    assert "has_any_column_privilege(" in plan
    assert "'INSERT,UPDATE,REFERENCES'" in plan
    assert "'SELECT,INSERT,UPDATE,REFERENCES'" in bootstrap.BACKUP_VERIFY_SQL

    backup_only_plan = "\n".join(
        (
            bootstrap.BACKUP_DATABASE_AND_SCHEMA_SQL,
            bootstrap.BACKUP_GRANTS_SQL,
            bootstrap.BACKUP_VERIFY_SQL,
        )
    )
    assert "REVOKE CREATE, TEMPORARY ON DATABASE %I FROM PUBLIC" in backup_only_plan
    assert "REVOKE CREATE ON SCHEMA public FROM PUBLIC" in backup_only_plan
    assert "REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM PUBLIC" in backup_only_plan
    assert "REVOKE ALL PRIVILEGES ON TYPE %I.%I FROM PUBLIC" in backup_only_plan
    assert "REVOKE ALL PRIVILEGES ON TYPE %I.%I FROM expenseops_backup" in backup_only_plan
    assert "type_object.typtype <> 'm'" in bootstrap.BACKUP_GRANTS_SQL
    assert "type_object.typtype <> 'm'" in bootstrap.BACKUP_VERIFY_SQL
    assert "default_transaction_read_only=on" in bootstrap.BACKUP_VERIFY_SQL
    assert "expenseops_backup has privileges on an unexpected schema" in backup_only_plan
    assert "backup has privileges on an unexpected relation" in backup_only_plan
    assert "backup has privileges on an unexpected sequence" in backup_only_plan
...
def test_default_invocation_is_non_connecting_secret_free_dry_run(monkeypatch, capsys):
    ...
    monkeypatch.setenv(bootstrap.BACKUP_PASSWORD_ENV, "backup-secret-must-not-print")
    ...
    assert "backup-secret-must-not-print" not in output
    assert bootstrap.RUNTIME_PASSWORD_ENV in output
    assert bootstrap.MIGRATOR_PASSWORD_ENV in output
    assert bootstrap.BACKUP_PASSWORD_ENV in output
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("backup", bootstrap.BACKUP_PASSWORD_ENV),
    ],
)
def test_apply_requires_all_environment_only_credentials(monkeypatch, missing, message):
    values = {
        ...
        bootstrap.BACKUP_PASSWORD_ENV: "backup-password-that-is-long-enough",
    }
    missing_env = {
        "admin": bootstrap.ADMIN_URL_ENV,
        "runtime": bootstrap.RUNTIME_PASSWORD_ENV,
...
def test_apply_uses_parameterized_passwords_and_verifies_before_commit(monkeypatch):
    ...
    backup_password = "backup-password-that-is-long-enough"
    ...
    assert backup_password not in sql_text
    assert (
    ...
    assert (
        (f"SELECT pg_catalog.set_config('{bootstrap.BACKUP_PASSWORD_SETTING}', %s, true)"),
        (backup_password,),
    ) in calls
...
def test_backup_only_bootstrap_is_parameterized_and_cannot_cut_over_other_roles(monkeypatch):
    calls: list[tuple[str, tuple[str, ...] | None]] = []
    connected: list[str] = []
    ...
    monkeypatch.setattr(bootstrap.psycopg, "connect", fake_connect)
    backup_password = "backup-password-that-is-long-enough"
    bootstrap.bootstrap_backup_role(
        admin_url="postgresql+psycopg://admin:private@example.test/expenseops",
        backup_password=backup_password,
    )
    ...
    assert calls[-1][0] == bootstrap.BACKUP_VERIFY_SQL
    backup_only_sql = "\n".join(
        (
            *bootstrap.BACKUP_ROLE_STATEMENTS,
            bootstrap.BACKUP_PASSWORD_SQL,
            bootstrap.BACKUP_DATABASE_AND_SCHEMA_SQL,
            bootstrap.BACKUP_GRANTS_SQL,
            bootstrap.BACKUP_VERIFY_SQL,
        )
    )
    assert bootstrap.RUNTIME_ROLE not in backup_only_sql
    assert bootstrap.MIGRATOR_ROLE not in backup_only_sql
    assert "ALTER DEFAULT PRIVILEGES" not in backup_only_sql
    assert "ALTER SCHEMA public OWNER" not in backup_only_sql
    assert "ALTER TABLE %I.%I OWNER" not in backup_only_sql
    assert "ALTER SEQUENCE %I.%I OWNER" not in backup_only_sql
    assert "REASSIGN OWNED" not in backup_only_sql
    assert "DROP OWNED" not in backup_only_sql


def test_backup_only_mode_requires_exact_environment_scope(monkeypatch, capsys):
    calls: list[tuple[str, str]] = []
    monkeypatch.setenv(
        bootstrap.ADMIN_URL_ENV,
        "postgresql://admin:private@example.test/expenseops",
    )
    monkeypatch.setenv(bootstrap.BACKUP_PASSWORD_ENV, "backup-password-that-is-long-enough")
    ...
    monkeypatch.setattr(
        ...
        lambda *, admin_url, backup_password: calls.append((admin_url, backup_password)),
    )

    assert bootstrap.main(["--bootstrap-backup-role"]) == 0
    assert calls == [
        (
            "postgresql://admin:private@example.test/expenseops",
            "backup-password-that-is-long-enough",
        )
    ]
    output = capsys.readouterr().out
    assert "backup database role" in output
    assert "private" not in output
    assert "backup-password-that-is-long-enough" not in output

    monkeypatch.setenv(bootstrap.RUNTIME_PASSWORD_ENV, "runtime-password-that-is-long-enough")
    with pytest.raises(SystemExit, match="must not be present"):
        bootstrap.main(["--bootstrap-backup-role"])
...
def test_reconcile_uses_only_database_url_and_no_admin_secrets(monkeypatch, capsys):
    ...
    monkeypatch.delenv(bootstrap.RUNTIME_PASSWORD_ENV, raising=False)
    monkeypatch.delenv(bootstrap.MIGRATOR_PASSWORD_ENV, raising=False)
    monkeypatch.delenv(bootstrap.BACKUP_PASSWORD_ENV, raising=False)
    monkeypatch.setattr(
        bootstrap,
...
def test_role_bootstrap_documentation_preserves_two_pass_and_backup_boundary():
    ...
    assert "encrypt the off-platform logical backup" in documentation_words
    assert "Railway PITR window" in documentation_words
    ...
    assert "`EXPENSEOPS_BACKUP_DATABASE_URL`" in documentation
    assert "`EXPENSEOPS_BACKUP_RECIPIENT_CERT_B64`" in documentation
    assert "`EXPENSEOPS_BACKUP_RECIPIENT_CERT_SHA256`" in documentation
    assert "EXPENSEOPS_BACKUP_ENCRYPTION_PASSPHRASE" not in documentation
    assert "public key material, not a secret" in documentation_words
    assert (
        "private key and its password must remain offline and outside GitHub" in documentation_words
    )
    assert "Do not store any backup credential on a Railway service" in documentation
    assert "A non-bypass login would silently produce an incomplete dump" in documentation
    assert "--bootstrap-backup-role" in documentation
    assert "does not create or alter the runtime/migrator roles" in documentation_words
    assert "transfer ownership, or mutate default ACLs" in documentation_words
    assert "It rejects runtime or migrator password variables" in documentation_words
...
def test_release_gate_runs_role_bootstrap_on_real_postgres_paths():
    ...
    assert "python scripts/bootstrap_database_roles.py --bootstrap-backup-role" in workflow

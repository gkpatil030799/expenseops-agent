...
| Unknown-share safety | Current and prior summaries independently count shared purchases and credits omitted for missing viewer allocation; the UI surfaces period-specific warnings and suppresses exact percentages for incomplete actual-share purchase comparisons instead of guessing |
| Accounting equation | `total_cents = personal_cents + shared_cents + unreviewed_cents` over eligible purchase rows; every primary amount and purchase count is non-negative, and `credits_cents` is a separate positive magnitude |
| Credit/refund semantics | Plaid-sign negative eligible rows are treated honestly as the broader credit class because the stored projection cannot reliably distinguish every merchant refund from another credit. Credits never reduce Total Spend, category/merchant rankings, or purchase trends. Card basis reports positive raw-card credit magnitude; actual-share basis reports only attributable personal/unreviewed credits and counts unallocatable shared credits separately for current and prior periods instead of guessing. |
| Reporting UI | Currency selection, excluded-currency scope, exact purchase Total reconciliation, separate Unreviewed and basis-labeled Credits KPIs, current/prior actual-share omission warnings, and viewer-identity guidance are visible in Insights |
| Weekly comparison truth | The six supported beta phrasings resolve to Monday-through-today versus the same weekdays seven days earlier; persisted ISO ranges and `same_weekdays_last_week` mode are code-owned, while qualified/custom requests are not widened or rewritten |
